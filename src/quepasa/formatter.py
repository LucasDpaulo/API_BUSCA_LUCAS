...
def formatar_relatorio(nome_empresa: str, report: ReportData) -> str:
    ...
    return (
        ...
        f"\n"
        f"\U0001f4b0 Financeiro Mensal:\n"
        f"  \u2022 Aberto: {_formatar_moeda(report.valor_boletos_abertos)}\n"
        f"  \u2022 Pago: {_formatar_moeda(report.valor_boletos_pagos)} ({report.percentual_conversao}%)"
    )
